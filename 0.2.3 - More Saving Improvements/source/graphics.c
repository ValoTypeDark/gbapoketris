...
void show_game_over(void) {
    ...
    int title_x = 60;
    int title_y = 15;  // Moved up from 20
    ...
    int stats_y = 45;  // Moved up from 50
    
    // SCORE
    draw_text_menu(40, stats_y, "SCORE:", WHITE);
    draw_number(120, stats_y, game.score, YELLOW);
    
    // LINES (tighter 14px spacing)
    draw_text_menu(40, stats_y + 14, "LINES:", WHITE);
    draw_number(120, stats_y + 14, game.lines_cleared, YELLOW);
    
    // LEVEL (added back)
    draw_text_menu(40, stats_y + 28, "LEVEL:", WHITE);
    draw_number(120, stats_y + 28, game.level, YELLOW);
    
    // POKEMON
    draw_text_menu(40, stats_y + 42, "POKEMON:", WHITE);
    draw_number(136, stats_y + 42, game.pokemon_caught_this_game, POKEMON_YELLOW);
    
    // NEW DEX
    draw_text_menu(40, stats_y + 56, "NEW DEX:", WHITE);
    draw_number(136, stats_y + 56, game.new_dex_entries, GREEN);
    
    // NEW HIGH SCORE indicator - use MENU font, centered, above buttons
    if(is_high_score(game.mode, game.score)) {
        int hs_y = stats_y + 75;  // y=120
        // "NEW HIGH SCORE" = 14 chars * 10px = 140px wide, center at (240-140)/2 = 50
        draw_text_menu(50, hs_y, "NEW HIGH SCORE", POKEMON_YELLOW);
    }
    
    // Menu options - at y=120, well separated from highscore
    int menu_y = 120;
    ...
    // Save status indicator - bottom left corner
    if(save_game_async_in_progress()) {
        draw_text_menu(8, 145, "SAVING...", GRAY);
    }
}
