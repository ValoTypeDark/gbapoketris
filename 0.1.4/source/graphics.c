...
void render_game(void) {
    ...
    if(game.state == STATE_GAMEPLAY || game.state == STATE_PAUSE) {
        ...
        // Draw particles (on top of everything, with shake offset)
        for(i = 0; i < MAX_PARTICLES; i++) {
            if(particles[i].active) {
                int px = particles[i].x + game.shake_offset_x;
                int py = particles[i].y + game.shake_offset_y;
                
                // Draw 2x2 particle
                if(px >= 0 && px < SCREEN_WIDTH-1 && py >= 0 && py < SCREEN_HEIGHT-1) {
                    back_buffer[py * SCREEN_WIDTH + px] = particles[i].color;
                    back_buffer[py * SCREEN_WIDTH + px + 1] = particles[i].color;
                    back_buffer[(py + 1) * SCREEN_WIDTH + px] = particles[i].color;
                    back_buffer[(py + 1) * SCREEN_WIDTH + px + 1] = particles[i].color;
                }
            }
        }
...
void show_game_over(void) {
    clear_screen(BLACK); // Black background (will add image later)
    
    // "GAME OVER" in Pokemon Solid 14pt - Red with black outline
    // Center at x=120 (half of 240)
    int title_x = 60;  // "GAME OVER" ~10 chars * 14pt = ~120px, centered
    int title_y = 20;
    ...
    // Stats section - use Pokemon Game 10pt
    int stats_y = 50;
    
    // Score
    draw_text_menu(40, stats_y, "SCORE:", WHITE);
    draw_number(120, stats_y, game.score, YELLOW);
    
    // Lines cleared
    draw_text_menu(40, stats_y + 15, "LINES:", WHITE);
    draw_number(120, stats_y + 15, game.lines_cleared, YELLOW);
    
    // Level
    draw_text_menu(40, stats_y + 30, "LEVEL:", WHITE);
    draw_number(120, stats_y + 30, game.level, YELLOW);
    
    // Pokemon caught
    draw_text_menu(40, stats_y + 45, "POKEMON:", WHITE);
    draw_number(120, stats_y + 45, game.pokemon_caught_this_game, POKEMON_YELLOW);
    
    // New Pokedex entries
    draw_text_menu(40, stats_y + 60, "NEW DEX:", WHITE);
    draw_number(120, stats_y + 60, game.new_dex_entries, GREEN);
    
    // Check if this is a highscore
    if(is_high_score(game.mode, game.score)) {
        draw_text_large(35, stats_y + 80, "NEW HIGHSCORE!", POKEMON_YELLOW);
    }
    ...
    if(game.game_over_selection == 0) {
        // Selected - yellow with outline
        draw_text_large(59, menu_y-1, "RESTART", BLACK);  // Outline
    ...
    if(game.game_over_selection == 1) {
        // Selected - yellow with outline
        draw_text_large(69, menu_y+16-1, "MENU", BLACK);  // Outline
        draw_text_large(71, menu_y+16-1, "MENU", BLACK);
        draw_text_large(69, menu_y+16+1, "MENU", BLACK);
        draw_text_large(71, menu_y+16+1, "MENU", BLACK);
        draw_text_large(70, menu_y+16, "MENU", POKEMON_YELLOW);
    } else {
        draw_text_large(70, menu_y+16, "MENU", WHITE);
    }
}
