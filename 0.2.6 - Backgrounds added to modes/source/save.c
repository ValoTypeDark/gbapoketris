...
void save_game_async_begin(void) {
    ...
    pending_save.modes_unlocked = current_save.modes_unlocked;
...
void unlock_mode(GameMode mode) { if(mode < MODE_COUNT) current_save.modes_unlocked |= (1 << mode); }
int is_mode_unlocked(GameMode mode) { return (mode < MODE_COUNT) && (current_save.modes_unlocked & (1 << mode)); }
