...
// Draw placed blocks with shake
        ...
        // Draw Pokemon sprite inside the box (software blit into back_buffer)
        {
            ...
            if(sprite) {
                /* Sprite positioned centered in Pokemon box */
                display_sprite_bg(poke_x, poke_y, sprite, 32, 32);
                /* Shiny star indicator */
                if(game.is_shiny) {
                    draw_text(poke_x + 24, poke_y + 2, "*", YELLOW);
                }
            }
        }

        // Pokemon name — uppercase, small font, mode-colored with black outline
        {
            ...
            if(pdata) {
                ...
                int name_y  = 115;  // Below the 32x32 sprite
