...
void show_bonus_select(void) {
    ...
    int text_width = text_len * 15;
    int text_x = (SCREEN_WIDTH - text_width) / 2;
    draw_text_large_custom_spacing(text_x, 30, bonus_names[current_mode], POKEMON_YELLOW, 12);
    
    // Draw description lines (centered, smaller font)
    int desc_y = 70;
    
    // Line 1
    text_len = 0;
    while(bonus_desc_line1[current_mode][text_len] != '\0') text_len++;
    text_width = text_len * 10;
    text_x = (SCREEN_WIDTH - text_width) / 2;
    draw_text_menu(text_x, desc_y, bonus_desc_line1[current_mode], WHITE);
    
    // Line 2
    if(bonus_desc_line2[current_mode][0] != '\0') {
        text_len = 0;
        while(bonus_desc_line2[current_mode][text_len] != '\0') text_len++;
        text_width = text_len * 10;
        text_x = (SCREEN_WIDTH - text_width) / 2;
        draw_text_menu(text_x, desc_y + 15, bonus_desc_line2[current_mode], WHITE);
    }
    
    // Navigation arrows
    int arrow_y = 28;
    if(current_mode > 0) {
        draw_text_menu(10, arrow_y, "<<", WHITE);
    }
    if(current_mode < 3) {
        draw_text_menu(SCREEN_WIDTH - 30, arrow_y, ">>", WHITE);
    }
    
    // Helper text
    draw_text(SCREEN_WIDTH/2 - 50, SCREEN_HEIGHT - 20, "A: SELECT  B: BACK", GRAY);
}
