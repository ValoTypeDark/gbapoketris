...
void show_bonus_select(void) {
    ...
    // Draw mode name in large outlined Pokemon font (centered)
    int text_len = 0;
    while(bonus_names[current_mode][text_len] != '\0') text_len++;
    int text_width = text_len * 15; // ~15px per char for Pokemon Solid with spacing=14
    int center_x = SCREEN_WIDTH/2 - text_width/2;
    
    // Use large outlined Pokemon font for mode name (matching mode select)
    draw_text_large_outlined_custom_spacing(center_x, 50, bonus_names[current_mode], POKEMON_BLUE, POKEMON_YELLOW, 14);
    
    // Draw description line 1 (centered)
    if(bonus_desc_line1[current_mode][0] != '\0') {
        int desc1_len = 0;
        while(bonus_desc_line1[current_mode][desc1_len] != '\0') desc1_len++;
        int desc1_width = desc1_len * 8;
        draw_text(SCREEN_WIDTH/2 - desc1_width/2, 75, bonus_desc_line1[current_mode], WHITE);
    }
    
    // Draw description line 2 (centered)
    if(bonus_desc_line2[current_mode][0] != '\0') {
        int desc2_len = 0;
        while(bonus_desc_line2[current_mode][desc2_len] != '\0') desc2_len++;
        int desc2_width = desc2_len * 8;
        draw_text(SCREEN_WIDTH/2 - desc2_width/2, 85, bonus_desc_line2[current_mode], WHITE);
    }
    
    // Draw simple arrows (matching mode select)
    if(current_mode > 0) {
        // Left arrow - use Pokemon Solid font
        draw_text_large_outlined_custom_spacing(10, 55, "<", POKEMON_BLUE, POKEMON_YELLOW, 14);
    }
