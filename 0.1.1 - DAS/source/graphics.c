...
void render_game(void) {
    ...
    if(game.state == STATE_GAMEPLAY || game.state == STATE_PAUSE) {
        ...
        if(game.hold_piece.type != TETRO_COUNT) {
            // Draw at proper scale in the hold box (8x8 blocks)
            for(int i = 0; i < 4; i++) {
                for(int j = 0; j < 4; j++) {
        ...
        // RIGHT SIDE UI (board ends at x=130, so start at x=135)
        // Next piece section
        // "NEXT" = 4 chars * 8 pixels = 32 pixels
        draw_text(185, 10, "NEXT", WHITE);
        // Draw next piece box - larger box now (32x32 to match bigger blocks)
        int next_x = 180;
        int next_y = 20;
        for(int i = 0; i < 33; i++) {
        ...
        if(game.next_piece.type != TETRO_COUNT) {
            // Draw at proper scale in the next box (8x8 blocks)
            for(int i = 0; i < 4; i++) {
                for(int j = 0; j < 4; j++) {
        ...
        draw_text(175, 80, "POKEMON", WHITE);
        // Draw Pokemon box (32x32 sprite area)
        int poke_x = 180;
        int poke_y = 95;
        for(int i = 0; i < 33; i++) {
