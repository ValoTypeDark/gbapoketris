...
typedef struct {
    ...
    u8 control_swap;              // 0=L=Hold/R=Flip, 1=L=Flip/R=Hold
    
    // Mode unlock tracking (replaces single bitflag)
    u8 mode_unlocked[8];          // Individual unlock status for each mode
...
void save_caught_pokemon(u16 index, u8 is_shiny);   // Save one catch at moment of catch
void save_highscore(GameMode mode, u32 score, const u8* mode_unlocked); // Save score at game over
