...
void save_pokemon_progress_deferred(const PokemonCatch* in_catches) {
    if(!in_catches) return;
    // Zero pending_save pokemon arrays first to prevent stale EWRAM garbage
    memset(pending_save.pokemon_unlocked, 0, sizeof(pending_save.pokemon_unlocked));
    memset(pending_save.pokemon_unlocked_shiny, 0, sizeof(pending_save.pokemon_unlocked_shiny));
    memset(pending_save.catch_count, 0, sizeof(pending_save.catch_count));
    memset(pending_save.catch_count_shiny, 0, sizeof(pending_save.catch_count_shiny));
    current_save.total_pokemon_unlocked = 0;
    current_save.total_pokemon_unlocked_shiny = 0;
...
// Save a single caught Pokemon immediately to SRAM.
// Called from unlock_pokemon() at the moment of catch.
void save_caught_pokemon(u16 index, u8 is_shiny) {
    if(index >= TOTAL_POKEMON) return;
    if(is_shiny) {
        current_save.pokemon_unlocked_shiny[index] = 1;
        current_save.catch_count_shiny[index]++;
        current_save.total_pokemon_unlocked_shiny = 0;
        for(int i = 0; i < TOTAL_POKEMON; i++)
            if(current_save.pokemon_unlocked_shiny[i])
                current_save.total_pokemon_unlocked_shiny++;
    } else {
        current_save.pokemon_unlocked[index] = 1;
        current_save.catch_count[index]++;
        current_save.total_pokemon_unlocked = 0;
        for(int i = 0; i < TOTAL_POKEMON; i++)
            if(current_save.pokemon_unlocked[i])
                current_save.total_pokemon_unlocked++;
...
// Save highscore and mode unlocks at game over.
void save_highscore(GameMode mode, u32 score, const u8* mode_unlocked) {
    add_high_score(mode, score);
    if(mode_unlocked)
        memcpy(current_save.mode_unlocked, mode_unlocked, 8);
    save_game();
}
