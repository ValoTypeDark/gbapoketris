...
// OPTIMIZATION 6: Menu background cache - avoids redrawing static menu BG every frame
static u16 menu_bg_cache[SCREEN_WIDTH * SCREEN_HEIGHT] __attribute__((section(".ewram")));
static int menu_bg_cached = 0;
...
// OPTIMIZATION 6: Cache the menu background to avoid redundant bitmap copies
void draw_menu_background(void) {
    // Build cache on first use
    if(!menu_bg_cached) {
        u32* dst32 = (u32*)menu_bg_cache;
        const u32* src32 = (const u32*)menu_bg;
        
        // Copy menu_bg bitmap to cache once
    ...
    const u32* src32 = (const u32*)menu_bg_cache;
...
// Draw placed blocks with shake
        ...
        if(game.state == STATE_GAMEPLAY) {
            // Calculate ghost piece position by simulating drop
            Tetromino ghost = game.current_piece;
            while(1) {
                ghost.y++;
                if(check_collision(&ghost, &game.board)) {
                    ghost.y--;
                    break;
                }
            }
            
            // Draw ghost piece with semi-transparent effect (darker version of piece color)
            for(int i = 0; i < ghost.block_count; i++) {
                int x = board_draw_x + (ghost.x + ghost.blocks[i][0]) * BLOCK_SIZE;
                int y = board_draw_y + (ghost.y + ghost.blocks[i][1]) * BLOCK_SIZE;
                
                // Create darkened/dimmed version of the piece color (30% brightness)
                u16 original = ghost.color;
                int r = (original & 0x1F);
                int g = ((original >> 5) & 0x1F);
                int b = ((original >> 10) & 0x1F);
                ...
                u16 ghost_color = RGB15(r, g, b);
                
                // Draw ghost block (just outline, or filled with dimmed color)
                for(int dy = 0; dy < BLOCK_SIZE; dy++) {
                    for(int dx = 0; dx < BLOCK_SIZE; dx++) {
                        int px = x + dx;
                        int py = y + dy;
                        if(px >= 0 && px < SCREEN_WIDTH && py >= 0 && py < SCREEN_HEIGHT) {
                            // Draw outline only (hollow ghost)
                            if(dx == 0 || dx == BLOCK_SIZE-1 || dy == 0 || dy == BLOCK_SIZE-1) {
                                back_buffer[py * SCREEN_WIDTH + px] = ghost_color;
                            }
                        }
                    }
                }
        ...
        if(game.state == STATE_GAMEPLAY) {
            // Calculate ghost piece position (drop current piece until collision)
            Tetromino ghost = game.current_piece;
            while(1) {
                ghost.y++;
                if(check_collision(&ghost, &game.board)) {
                    ghost.y--;  // Back up one row
                    break;
                }
            }
            
            // Only draw ghost if it's below current piece (not at same position)
            if(ghost.y > game.current_piece.y) {
                // Draw ghost piece with semi-transparent effect (lighter color)
                for(int i = 0; i < ghost.block_count; i++) {
                    int gx = board_draw_x + (ghost.x + ghost.blocks[i][0]) * BLOCK_SIZE;
                    int gy = board_draw_y + (ghost.y + ghost.blocks[i][1]) * BLOCK_SIZE;
                    
                    // Draw ghost block with outline only (hollow)
                    if(gx >= 0 && gy >= 0 && gx + BLOCK_SIZE <= SCREEN_WIDTH && gy + BLOCK_SIZE <= SCREEN_HEIGHT) {
                        // Make ghost color lighter/semi-transparent (multiply RGB by 0.5 + add white)
                        u16 base_color = ghost.color;
                        int r = (base_color & 0x1F);
                        int g = ((base_color >> 5) & 0x1F);
                        int b = ((base_color >> 10) & 0x1F);
                        
                        // Lighten: average with white
                        r = (r + 31) / 2;
                        g = (g + 31) / 2;
                        b = (b + 31) / 2;
                        u16 ghost_color = (b << 10) | (g << 5) | r;
                        
                        // Draw hollow outline (2px border)
                        for(int dy = 0; dy < BLOCK_SIZE; dy++) {
                            for(int dx = 0; dx < BLOCK_SIZE; dx++) {
                                int px = gx + dx;
                                int py = gy + dy;
                                
                                // Only draw border pixels (hollow center)
                                if(dx < 2 || dx >= BLOCK_SIZE-2 || dy < 2 || dy >= BLOCK_SIZE-2) {
                                    if(px >= 0 && px < SCREEN_WIDTH && py >= 0 && py < SCREEN_HEIGHT) {
                                        back_buffer[py * SCREEN_WIDTH + px] = ghost_color;
                                    }
                                }
        ...
        } else if(game.next_piece.category == PIECE_UNOWN) {
            // Unown piece in next
            for(int i = 0; i < game.next_piece.block_count; i++) {
                int px = next_x + game.next_piece.blocks[i][0] * 6;
                int py = next_y + game.next_piece.blocks[i][1] * 6;
        ...
        } else if(game.hold_piece.category == PIECE_UNOWN) {
            // Unown piece in hold
            for(int i = 0; i < game.hold_piece.block_count; i++) {
                int px = hold_x + game.hold_piece.blocks[i][0] * 6;
                int py = hold_y + game.hold_piece.blocks[i][1] * 6;
        ...
        // Check if Pokemon has been caught (check in-memory game state, not saved state)
        int is_caught = 0;
        if(game.current_pokemon < TOTAL_POKEMON) {
            // Check if unlocked in current game session (either normal or shiny form)
            is_caught = game.pokemon_catches[game.current_pokemon].unlocked || 
                        game.pokemon_catches[game.current_pokemon].unlocked_shiny;
        }
        ...
        // Mode-colored with black outline
        {
            ...
            if(pdata) {
                /* Check if caught in current game session (in-memory, not saved) */
                int is_caught = 0;
                if(game.current_pokemon < TOTAL_POKEMON) {
                    is_caught = game.pokemon_catches[game.current_pokemon].unlocked || 
                                game.pokemon_catches[game.current_pokemon].unlocked_shiny;
    ...
    if(game.state == STATE_PAUSE) {
        ...
        static u8 game_over_save_done = 0;
        
        // Save highscore on first frame of game over screen
        if(!game_over_save_done) {
            save_highscore(game.mode, game.score, game.mode_unlocked);
            game_over_save_done = 1;
        }
        
        // Reset flag when leaving game over screen
        if(game.state != STATE_GAME_OVER) {
            game_over_save_done = 0;
...
void show_pause_menu(void) {
    // Pause menu box (similar style to catch celebration)
    int box_width = 160;
    int box_height = 90;
    int box_x = (SCREEN_WIDTH - box_width) / 2;
    int box_y = (SCREEN_HEIGHT - box_height) / 2;
    
    // Draw box background (solid dark blue)
    for(int y = box_y; y < box_y + box_height; y++) {
        for(int x = box_x; x < box_x + box_width; x++) {
            back_buffer[y * SCREEN_WIDTH + x] = RGB15(3, 3, 8);
        }
    }
    ...
    for(int x = box_x; x < box_x + box_width; x++) {
        if(x >= 0 && x < SCREEN_WIDTH) {
            if(box_y >= 0 && box_y < SCREEN_HEIGHT) {
                back_buffer[box_y * SCREEN_WIDTH + x] = WHITE;
            }
            if(box_y + box_height - 1 >= 0 && box_y + box_height - 1 < SCREEN_HEIGHT) {
                back_buffer[(box_y + box_height - 1) * SCREEN_WIDTH + x] = WHITE;
            }
        }
    }
    for(int y = box_y; y < box_y + box_height; y++) {
        if(y >= 0 && y < SCREEN_HEIGHT) {
            if(box_x >= 0 && box_x < SCREEN_WIDTH) {
                back_buffer[y * SCREEN_WIDTH + box_x] = WHITE;
            }
            if(box_x + box_width - 1 >= 0 && box_x + box_width - 1 < SCREEN_WIDTH) {
                back_buffer[y * SCREEN_WIDTH + (box_x + box_width - 1)] = WHITE;
            }
        }
    }
    
    // Draw "PAUSED" title centered at top
    int title_y = box_y + 12;
    // "PAUSED" = 6 characters, large font is ~16px per character
    int title_width = 6 * 16;  // Approximate width
    int title_x = box_x + (box_width - title_width) / 2;
    draw_text_large(title_x, title_y, "PAUSED", POKEMON_YELLOW);
    
    // Menu options
    int option_y = box_y + 40;
    int spacing = 20;
    
    // Resume option
    u16 resume_color = (game.pause_menu_selection == 0) ? POKEMON_YELLOW : WHITE;
    int resume_x = box_x + 45;
    draw_text_menu(resume_x, option_y, "RESUME", resume_color);
    if(game.pause_menu_selection == 0) {
        draw_text_menu(resume_x - 15, option_y, ">", POKEMON_YELLOW);
    }
    
    // Quit option
    u16 quit_color = (game.pause_menu_selection == 1) ? POKEMON_YELLOW : WHITE;
    int quit_x = box_x + 45;
    draw_text_menu(quit_x, option_y + spacing, "QUIT", quit_color);
    if(game.pause_menu_selection == 1) {
        draw_text_menu(quit_x - 15, option_y + spacing, ">", POKEMON_YELLOW);
    }
}
...
void show_game_over(void) {
    // Draw the game over background bitmap
    u32* dst32 = (u32*)back_buffer;
    const u32* src32 = (const u32*)game_over_bg;  // Your new background
    
    // Fast copy: 38400 pixels = 19200 u32 copies
    for(int i = 0; i < 19200; i++) {
        dst32[i] = src32[i];
    }
    
    // Reset flag when leaving game over (called from reset_game_over_state)
    // Nothing to check for async anymore - save is instant
    
    // Check if this is a new highscore
    int is_new_highscore = is_high_score(game.mode, game.score);
    
    // "GAME OVER" text is now in the background image, no need to draw it
    
    // NEW HIGHSCORE message if applicable (ALL CAPS, WHITE TEXT)
    if(is_new_highscore) {
        // "NEW HIGHSCORE!" centered
        int hs_x = 50;
        int hs_y = 40;
        draw_text_menu(hs_x, hs_y, "NEW HIGHSCORE!", POKEMON_YELLOW);
    }
    
    // Stats section - ALL CAPS, WHITE/YELLOW text (NO BLACK)
    int stats_y = is_new_highscore ? 65 : 55;
    int label_x = 30;
    int value_x = 110;
    
    // POINTS: (white label, yellow value)
    draw_text(label_x, stats_y, "POINTS:", WHITE);
    draw_number(value_x, stats_y, game.score, YELLOW);
    
    // LINES: (white label, yellow value)
    draw_text(label_x, stats_y + 15, "LINES:", WHITE);
    draw_number(value_x, stats_y + 15, game.lines_cleared, YELLOW);
    
    // CAUGHT: (white label, yellow value)
    draw_text(label_x, stats_y + 30, "CAUGHT:", WHITE);
    draw_number(value_x, stats_y + 30, game.pokemon_caught_this_game, YELLOW);
    
    // NEW: (white label, green value)
    draw_text(label_x, stats_y + 45, "NEW:", WHITE);
    draw_number(value_x, stats_y + 45, game.new_dex_entries, GREEN);
    
    int bottom_y = 130;
    
    // Show "RESTART" and "MENU" options - ALL CAPS
    int restart_x = 40;
    int menu_x = 150;
    
    // RESTART option (ALL CAPS)
